...
After becoming famous, the CodeBots decided to move into a new building together. Each of the rooms has a different cost,
 and some of them are free, but there's a rumour that all the free rooms are haunted! Since the CodeBots are quite 
...
function matrixElementsSum(matrica) {
    var sum =0;
     
            for (n=0; n <matrica.length; n++) {
               
                for(var i=0; i<=matrica[n].length;i++){
                    if(matrica[n][i]<=0){
            
                        for(var j=n;j<matrica.length;j++){
                            matrica[j][i]=0;
                        }
                    } 
                  }
    
                  for(var i=0; i<matrica[0].length;i++){
                    sum += matrica[n][i] ;
                }
